...
class InstrSrc(Screen):
    def __init__(self,**kwargs):
        super().__init__(**kwargs)
        textsrc = Label(text = txt_instruction) 
        lbl1 = Label(text = 'Введите имя:',haling = 'right')
        self.in_name = TextInput(multiline = False)
        lbl2 = Label(text = 'Введите возраст:',haling = 'right')
        self.in_age = TextInput(text = '7',multiline = False)
        self.btn = Button(text = 'Начать' , size_hint = (0.3 , 0.2), pos_hint = {'center_x':0.5})
        self.btn.on_press = self.next
        line1  = BoxLayout(size_hint = (0.8, None),hieght = '30sp')
        line2  = BoxLayout(size_hint = (0.8, None),hieght = '30sp')
        line1.add_widget(lbl1)
        line1.add_widget(self.in_name)
        line2.add_widget(lbl2)
        line2.add_widget(self.in_age)
        outer = BoxLayout(orientation = 'vertical',padding = 8,spacing = 8)
        outer.add_widget(textsrc)
...
class PulseScr2(Screen):
    def _init_(self,**kwargs):
        ...
        lbl_result1 = Label (text= 'Введите результат: ', halign= 'right' )
        ...
        self.btn = Button (text= "Завершить", size_hint=(0.3, 0.2), pos_hint={'center _x': 0.5})
        self.btn.on_press = self.next
        outer = BoxLayout (orientation='vertical', padding=8, spacing=8)
        outer.add_widget(instr)
        outer.add_widget(line1)
    ...
    def next (self):
        if not self.next_screen:
            ...
            global p2, p3
            p2 = check_int(self.in_resulti.text)
...
class HeartCheck(App):
    def build(self):
        Sm = ScreenManager()
        sm.add_widget (Insteser (name='instr'))
